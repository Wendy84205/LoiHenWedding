...
const elementOptions = [
  ...
  ['carousel', 'Trình chiếu', GalleryHorizontal], ['particle', 'Lấp lánh', Sparkles],
];
...
export function SceneElementsPanel({ scene, onAdd }) {
  return <section className="sceneElementsPanel"><header><Sparkles /><div><small>THÀNH PHẦN</small><strong>Thêm vào thiệp</strong></div></header><div>{elementOptions.map(([type, label, Icon]) => <button type="button" onClick={() => onAdd(createDefaultSceneNode(type, scene))} key={type}><Icon /><span>{label}</span></button>)}</div></section>;
}
