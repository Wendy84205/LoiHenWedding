alter table public.invitations add column if not exists design jsonb;
alter table public.invitations add column if not exists draft_design jsonb;
alter table public.invitation_versions add column if not exists design jsonb;

drop function if exists public.save_invitation_draft(uuid, integer, jsonb, jsonb, text);
...
drop function if exists public.switch_invitation_template(uuid, text, text);
...
begin
  ...
  update public.orders set template_slug = p_template_slug where id = p_order_id;
  update public.invitations
  ...
  values (
    ...
    current_invitation.draft_content, current_invitation.draft_theme, current_invitation.draft_design,
...
revoke all on function public.switch_invitation_template(uuid, text, jsonb, text) from public, anon, authenticated;
grant execute on function public.switch_invitation_template(uuid, text, jsonb, text) to service_role;
...
begin
  ...
  set template_slug = coalesce(target_version.template_slug, current_invitation.template_slug),
      ...
      draft_design = target_version.design,
      draft_version = draft_version + 1
  where id = current_invitation.id
  returning * into current_invitation;
